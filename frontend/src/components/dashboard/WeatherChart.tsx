...
function WeatherChartInner({ data }: WeatherChartProps) {
  return (
    <div className="w-full" style={{ flex: "1 1 0%", minHeight: 0 }} role="img" aria-label="Weather forecast chart">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} barCategoryGap="20%" margin={{ top: 8, right: 8, left: -10, bottom: 0 }}>
          ...
          />
          <Bar dataKey="rain" fill="#3B82F6" radius={[3, 3, 0, 0]} name="Rain (mm)" />
          <Bar dataKey="humidity" fill="#22C55E" radius={[3, 3, 0, 0]} name="Humidity (%)" />
          <Bar dataKey="wind" fill="#F59E0B" radius={[3, 3, 0, 0]} name="Wind (km/h)" />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
